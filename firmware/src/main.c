...
void play_song_mario(void);
void play_song_asa_branca(void);
...
void play_song_mario(){ //futuramente passar a musica aqui (nome da op??o)
	int notes = sizeof(melody_mario) / sizeof(melody_mario[0]) / 2; //tamanho total/tamanho int *0.5 (pq metade ? o tempo das notas)
	for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
...
void play_song_asa_branca(){ //futuramente passar a musica aqui (nome da op??o)
	// this calculates the duration of a whole note in ms
	...
	// sizeof gives the number of bytes, each int value is composed of two bytes (16 bits)
	// there are two values per note (pitch and duration), so for each note there are four bytes
	int notes = sizeof(melody_asa_branca) / sizeof(melody_asa_branca[0]) / 2; //tamanho total/tamanho int *0.5 (pq metade ? o tempo das notas)
	for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
		if (but_change_song_flag){
			break;
		}
		...
		// calculates the duration of each note
		divider = melody_asa_branca[thisNote + 1];
		noteDuration = (wholenote) / abs(divider);
		if (divider < 0) {
			noteDuration *= 1.5; // increases the duration in half for dotted notes
		}
		// we only play the note for 90% of the duration, leaving 10% as a pause
		tone(melody_asa_branca[thisNote], noteDuration * 0.9);
		pio_clear(LED_PIO, LED_PIO_IDX_MASK);
